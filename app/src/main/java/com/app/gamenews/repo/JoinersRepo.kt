package com.app.gamenews.repo

import com.app.gamenews.model.User
import com.google.firebase.firestore.DocumentSnapshot
import com.google.firebase.firestore.FirebaseFirestore
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.StateFlow

class JoinersRepo {

    private val db = FirebaseFirestore.getInstance()
    private val pageSize = 10
    private var lastVisible: DocumentSnapshot? = null
    private var isFetching = false
    private val list = ArrayList<User>()
    private val _listState = MutableStateFlow<List<User>>(emptyList())
    val listState: StateFlow<List<User>> get() = _listState


    fun getJoinerIds(roomId: String, onComplete: (List<String>) -> Unit, onError: (Exception) -> Unit) {
        val followersCollection = FirebaseFirestore.getInstance()
            .collection("Rooms")
            .document(roomId)
            .collection("Users")

        followersCollection.addSnapshotListener { querySnapshot, exception ->
            if (exception != null) {
                onError(exception) // Invoke onError in case of an exception
                return@addSnapshotListener
            }

            if (querySnapshot != null) {
                val joinerIds = mutableListOf<String>()
                for (document in querySnapshot.documents) {
                    val joinerId = document.id
                    joinerIds.add(joinerId)
                }
                onComplete(joinerIds) // Invoke onComplete with the list of follower IDs
            }
        }
    }

    fun fetchInitialData(followerIds: List<String>) {
        if (!isFetching) {
            isFetching = true
            val query = db.collection("Users")
                .whereIn("userId",followerIds)
                .limit(pageSize.toLong())

            query.addSnapshotListener { snapshot, exception ->
                if (exception != null) {
                    // Handle exception
                    return@addSnapshotListener
                }

                if (snapshot != null && !snapshot.isEmpty) {
                    list.clear()
                    for (document in snapshot.documents) {
                        val user = document.toObject(User::class.java)
                        user?.let { list.add(it) }
                        println("listemm $list")
                    }
                    lastVisible = snapshot.documents[snapshot.size() - 1]
                    _listState.value = list.toList()
                }
                isFetching = false
            }
        }
    }







}